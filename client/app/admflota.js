...
const registerDriver = async (event) => {
  ...
  // Mapeo básico de roles para asegurar que "admin" IT sea ID 1, y demás roles.
  let role_id = 3; // conductor por defecto
  if (roleName === 'admin') role_id = 1;
  else if (roleName === 'mantenimiento') role_id = 4;
  else if (roleName === 'it_tech') role_id = 5;
